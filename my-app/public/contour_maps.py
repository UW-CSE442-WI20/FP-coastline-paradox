...
IMG_PATH = "./maps/britain.png"  # we're only going to use this one source image of great britain
...
def main(args):
    ...
    ch = contours, hierarchy = cv2.findContours(
        threshold.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)

    if GENERATE:
        # generate once for target length of image
        result_img, result_target_len, result_approx_len = generate_approx(
            img.copy(), contours, EPSILON)
        ...
        print("Generating ", num_steps, " images using epsilon step size ",
              STEP_SIZE)
        epsilons = [STEP_SIZE * i for i in range(0, num_steps)]
        for e in epsilons:
            result_img, result_target_len, result_approx_len = generate_approx(
                img.copy(), contours, e)
            image_name = "/" + "epsilon_" + str(e).replace(".", "") + ".png"
